...
public class DashboardPanel extends JPanel {
    ...
    public DashboardPanel() {
        ...
        // === Cards superiores (estilo Reportes) ===
        ...
        tblExp.getColumnModel().getColumn(2).setCellRenderer(new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value,
                ...
                if (value instanceof LocalDate e) {
                    ...
                    if (!isSelected) {
                        if (dias <= 7) c.setForeground(new Color(0xDC2626));      // rojo fuerte
                        else if (dias <= 30) c.setForeground(new Color(0xD97706)); // naranja
    ...
    public void recargar() {
        lblHoy.setText(CLP.format(VentasDao.totalHoy()));
        lblSemana.setText(CLP.format(VentasDao.totalSemana()));
        lblMes.setText(CLP.format(VentasDao.totalMes()));

        List<Product> productos = InventoryDao.getAll();
        lblProductos.setText(String.valueOf(productos.size()));

        ((LowModel) tblLow.getModel()).set(
                productos.stream().filter(Product::hasLowStock).collect(Collectors.toList())
        );

        ((ExpModel) tblExp.getModel()).set(
                productos.stream()
                        .filter(p -> p.getExpiry() != null &&
                                !p.isExpired() &&
                                java.time.temporal.ChronoUnit.DAYS.between(LocalDate.now(), p.getExpiry()) <= 30)
                        .collect(Collectors.toList())
        );

        ((VentasModel) tblVentas.getModel()).set(VentasDao.listRecent(10));
    }
