...
/**
 * InventoryStatsDao ----------------- DAO dedicado únicamente a *estadísticas
 * de inventario*.
 *
 * ❗ Importante: - NO modifica datos - Solo consulta información - Se usa
 * principalmente en DashboardPanel
 */
public class InventoryStatsDao {

	/**
	 * lowStock(int threshold) ----------------------- Retorna productos cuyo stock
	 * es ≤ al threshold (ej: <=5).
	 */
	...
	/**
	 * expiringInDays(int days) ------------------------ Busca productos que vencen
	 * dentro de N días. Ignora productos con expiry NULL.
	 */
	...
	/**
	 * map(ResultSet rs) ----------------- Convierte una fila del SELECT en un
	 * objeto Product.
	 */
	private static Product map(ResultSet rs) throws SQLException {
		return new Product(rs.getInt("id"), // ID interno
	...
	/**
	 * parseDate(String s) ------------------- Convierte un String (YYYY-MM-DD) en
	 * LocalDate. Si está vacío o está mal formateado → retorna null.
	 */
